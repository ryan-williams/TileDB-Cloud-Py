...
def apply_base(
    ...
    **Example**
    >>> import tiledb, tiledb_cloud, numpy
    >>> def median(df):
    ...   return numpy.median(df["a"])
    >>> # Open the array then run the UDF
    >>> tiledb_cloud.array.apply_base("tiledb://TileDB-Inc/quickstart_dense", median, [(0,5), (0,5)], attrs=["a", "b", "c"]).result
    2.0
    """  # noqa: E501
    del v2  # unused
...
def apply(*args, **kwargs) -> Any:
    ...
    **Example:**
    ...
    ...   return numpy.median(df["a"])
    >>> # Open the array then run the UDF
    >>> tiledb_cloud.array.apply("tiledb://TileDB-Inc/quickstart_dense", median, [(0,5), (0,5)], attrs=["a", "b", "c"])
    2.0
    """  # noqa: E501
